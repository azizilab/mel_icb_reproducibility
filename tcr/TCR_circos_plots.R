### title: Create circos plots showing overlap in TCR clonotypes between matched samples
...
if (useRibas) {
  ...
  table_bc_IG = read.table("table_bc_IG.csv", header=T, sep=",", quote=NULL)
  sample_names = unique(table_bc_IG$sample)
  patslist = list()
  shortpatslist = list()
  colorslist = list()
  for (z in 1:length(sample_names))
  {
    asample = sample_names[z]
    subtable_sample = subset(table_bc_IG, sample==asample)
    cell_types_fine = unique(subtable_sample$cell_type_fine)
    patslist = list.append(patslist,cell_types_fine)
    shortpatslist = list.append(shortpatslist,cell_types_fine)
    colorslist = list.append(colorslist,rep("black",length(patslist)))#colorRampPalette(rev(brewer.pal(n = 7, name = "RdYlBu")))(length(patslist))
  }
} else {
...
if (useRibas) {
  ...
  pdf("TCR_circos_for_jana.pdf",height=10,width=10)
} else {
  if (fullcolors) {
...
for (z in 1:length(patslist))
{
  ...
  {
    table_bc_IG = read.table("table_bc_IG.csv", header=T, sep=",", quote=NULL)
    subtable_sample = subset(table_bc_IG, sample==sample_names[z])
    cell_types_fine = pats
    ...
    rlelist_noncum = list()
    for (acelltype in cell_types_fine)
    {
      subtable = subset(subtable_sample, cell_type_fine==acelltype)

      rleresult = rle(sort(subtable$heavy_light))$lengths
      names(rleresult) = rle(sort(subtable$heavy_light))$values
    ...
    names(rlelist) = cell_types_fine
    names(frequentrlelist) = cell_types_fine
    names(rlelist_noncum) = cell_types_fine
  } else {
    contig_list<-c()
  ...
  common_clonotypes_avg_freqs = rep(0,length(common_clonotypes_all))
  names(common_clonotypes_avg_freqs) = common_clonotypes_all
  for (aclonotype in common_clonotypes_all)
  {
    afreq = 0
    for (atype in names(rlelist))
    {
      if (sum(names(rlelist_noncum[[atype]])==aclonotype)!=0)
      {
        afreq = afreq + rlelist_noncum[[atype]][aclonotype]/sum(rlelist_noncum[[atype]])
      }
      else
      {
        afreq = afreq+0
    ...
    afreq = afreq / length(names(rlelist))
    common_clonotypes_avg_freqs[aclonotype] = afreq
  }

  common_clonotypes_avg_freqs = sort(common_clonotypes_avg_freqs, decreasing=TRUE)
  matchidxs = match(names(common_clonotypes_avg_freqs)[1:min(5,length(common_clonotypes_avg_freqs))], common_clonotypes_all)
  colorvec = common_colors_all[matchidxs]

  lgd_points = Legend(at = names(common_clonotypes_avg_freqs)[1:min(5,length(common_clonotypes_avg_freqs))], type = "points", legend_gp = gpar(col = colorvec), title_position = "topleft", title = "Most Common Clonotypes")
  lgd_list_vertical = packLegend(lgd_points)
  draw(lgd_list_vertical, x = unit(4, "mm"), y = unit(4, "mm"), just = c("left","bottom"))

  if (sample_names[z]=="F16_pre")
  {
    # nonsense = nonsense+1
  }
}
