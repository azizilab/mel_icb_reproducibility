...
#### Rerun KINOMO for unclear samples using ranks 2-10
...
#Instructions
#The KINOMO repository can be accessed via https://github.com/IzarLab/KINOMO.git
...
for(myrank in seq(2,10)){
  #Perform factorization using estimated rank
  nmf.estimate.rank <- nmf(mat, rank=myrank)
  saveRDS(nmf.estimate.rank,paste0(directory,pat,"_KINOMO_nmf_rank_",myrank,".rds"))
  ...
  write.csv(tab,paste0(directory,pat, "_KINOMO_nmf_rank_",myrank,"_top30_W.csv"))
  for(c in 1:ncol(tab)){
    seu1<-AddModuleScore(object = seu1,features = list(tab[,c]),
                         name = colnames(tab)[c],assay = 'RNA',search=T)
  }
  genelist <- c(tab[,1:myrank])
  ...
  write.csv(tab1,paste0(directory,pat, "_KINOMO_nmf_rank_",myrank,"_top50_W.csv"))
  for(c in 1:ncol(tab1)){
    seu2<-AddModuleScore(object = seu2,features = list(tab1[,c]),
                         name = colnames(tab1)[c],assay = 'RNA',search=T)
  }
  genelist1 <- c(tab1[,1:myrank])
  ...
  write.csv(tab2,paste0(directory,pat, "_KINOMO_nmf_rank_",myrank,"_top100_W.csv"))
  for(c in 1:ncol(tab2)){
    seu3<-AddModuleScore(object = seu2,features = list(tab2[,c]),
                         name = colnames(tab2)[c],assay = 'RNA',search=T)
  }
  genelist2 <- c(tab2[,1:myrank])
  ...
  write.csv(tab3,paste0(directory,pat, "_KINOMO_nmf_rank_",myrank,"_top200_W.csv"))
  for(c in 1:ncol(tab3)){
    seu4<-AddModuleScore(object = seu3,features = list(tab3[,c]),
                         name = colnames(tab3)[c],assay = 'RNA',search=T)
  }
  genelist3 <- c(tab3[,1:myrank])
  ...
  write.csv(tab4,paste0(directory,pat, "_KINOMO_nmf_rank_",myrank,"_top300_W.csv"))
  for(c in 1:ncol(tab4)){
    seu5<-AddModuleScore(object = seu5,features = list(tab4[,c]),
                         name = colnames(tab4)[c],assay = 'RNA',search=T)
  }
  genelist4 <- c(tab4[,1:myrank])
  ...
  pdf(file = paste0(directory,pat, "_KINOMO_nmf_rank_",myrank,".pdf"),height = 20,width=20)
  
  print(FeaturePlot(seu1, features = c(paste0(pat,'_factor',seq(1,myrank),'1')),
                    min.cutoff = "q05", max.cutoff = "q95")& 
          scale_colour_gradientn(colours = rev(brewer.pal(11,'Spectral'))))
  print(FeaturePlot(seu2, features = c(paste0(pat,'_factor',seq(1,myrank),'1')),
                    min.cutoff = "q05", max.cutoff = "q95")& 
          scale_colour_gradientn(colours = rev(brewer.pal(11,'Spectral'))))
  print(FeaturePlot(seu3, features = c(paste0(pat,'_factor',seq(1,myrank),'1')),
                    min.cutoff = "q05", max.cutoff = "q95")& 
          scale_colour_gradientn(colours = rev(brewer.pal(11,'Spectral'))))
  print(FeaturePlot(seu4, features = c(paste0(pat,'_factor',seq(1,myrank),'1')),
                    min.cutoff = "q05", max.cutoff = "q95")& 
          scale_colour_gradientn(colours = rev(brewer.pal(11,'Spectral'))))
  print(FeaturePlot(seu5, features = c(paste0(pat,'_factor',seq(1,myrank),'1')),
                    min.cutoff = "q05", max.cutoff = "q95")& 
          scale_colour_gradientn(colours = rev(brewer.pal(11,'Spectral'))))
  
  dev.off()
  write.csv(res@fit@W,paste0(directory,pat, "_KINOMO_nmf_rank_",myrank,"_W.csv"))
  write.csv(res@fit@H,paste0(directory,pat, "_KINOMO_nmf_rank_",myrank,"_H.csv"))
  
  #Plot meta-genes heatmaps
  pdf(file=paste0(directory,pat, "_KINOMO_nmf_rank_",myrank,"_basismap_30.pdf"),width = 10, height = 10)
  basismap(res[rownames(res)%in%genelist,])
  dev.off()
  
  pdf(file=paste0(directory,pat, "_KINOMO_nmf_rank_",myrank,"_basismap_50.pdf"),width = 20, height = 20)
  ...
  pdf(file=paste0(directory,pat, "_KINOMO_nmf_rank_",myrank,"_basismap_100.pdf"),width = 30, height = 30)
  basismap(res[rownames(res)%in%genelist2,])
  dev.off()
  
  pdf(file=paste0(directory,pat, "_KINOMO_nmf_rank_",myrank,"_basismap_200.pdf"),width = 40, height = 40)
  basismap(res[rownames(res)%in%genelist3,])
  dev.off()
  
  pdf(file=paste0(directory,pat, "_KINOMO_nmf_rank_",myrank,"_basismap_300.pdf"),width = 50, height = 50)
