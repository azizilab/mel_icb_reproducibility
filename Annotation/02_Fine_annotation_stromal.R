...
#### Fine cell type annotation of stromal cells
...
seu <- subset(seu, cell_type_main %in% c('Stromal cells'))
...
anchors <- FindIntegrationAnchors(object.list = obj.list, dims = 1:d, k.filter = n)

# Integrate data sets
seu <- IntegrateData(anchorset = anchors, dims = 1:d,k.weight=n)
...
#seu<-subset(seu,integrated_snn_res.0.3 %notin% c(5) & tcr=='no_TCR') #rerun int
...
# Save obj
saveRDS(seu,paste0(path.ct,'data_',filename,'_20dim_v3.rds'))
...
print(DimPlot(seu, label = F,group.by = c('responder','mucosal','treated'),
              shuffle = T,raster = T,pt.size = 2,ncol = 2))
...
print(FeaturePlot(seu, features = c("proportion_scaled_cnv_avg", "MLANA", 'GFAP', "MOG"), 
                  min.cutoff = "q05",max.cutoff = "q95", order = T,raster = T,pt.size = 2))

# mesenchymal 'NT5E','ENG', 'CD44','ALCAM','NGFR','NANOG','POU5F1','SOX2','KIT'
print(FeaturePlot(seu, features = c("rna_NT5E", 'rna_ENG', "rna_NGFR", 'rna_ALCAM'), 
                  min.cutoff = "q05",max.cutoff = "q95", order = T,raster = T,pt.size = 2)&
        scale_colour_gradientn(colours = my_palette))

# CAFs 'FAP','COL1A1','COL1A2','COL3A1'
print(FeaturePlot(seu, features = c("rna_FAP", 'rna_COL11A1', "rna_CCL2", 'rna_CXCL12',
                                    'rna_POSTN','rna_TGFB1'), 
                  min.cutoff = "q05",max.cutoff = "q95", order = T,raster = T,pt.size = 2)&
        scale_colour_gradientn(colours = my_palette))

# vSM vs pericytes
print(FeaturePlot(seu, features = c('rna_CNN1','rna_MYLK','rna_ACTA2',
                                    'rna_PDGFRB','rna_RGS5','rna_ABCC9'), 
                  min.cutoff = "q05",max.cutoff = "q95", order = T,raster = T,pt.size = 2)&
        scale_colour_gradientn(colours = my_palette))


DotPlot(seu,assay = 'RNA',features = c('FAP','RGS5',
                                       'MMP11','CDH11','POSTN', #matrix cafs
                                       'PLA2G2A', 'CFD','C3','CD34','CXCL12','CD248', #inflammatory cafs
                                       'ACTA2','MCAM', #vascular cafs
                                       'PDPN','MME',"ENO1",'NDRG1','CA9','HSPH1', #tumor-like cafs
                                       'IDO1','MX2','MX1','ISG15','IFI6','COL1A1','COL1A2', #Interferon- response CAFs
                                       'CD74','HLA-DRB1','HLA-DRA', #Antigen- presenting CAFs
                                       'CCL21','CCL19', #Reticular-like CAFs
                                       'MKI67','MLANA'
                                       ),
...
group.by = 'integrated_snn_res.0.2', dot.scale = 5)+ scale_color_viridis() + coord_flip()+ RotatedAxis()+
  theme(axis.text.y = element_text(size=7))

DotPlot(seu,assay = 'RNA',features = paste0(colnames(sigs),1),
group.by = 'integrated_snn_res.0.2', dot.scale = 5)+ 
  scale_color_viridis() + coord_flip()+ RotatedAxis()+
  theme(axis.text.y = element_text(size=7))

DotPlot(seu,assay = 'RNA',features = c('FAP','COL1A1','COL1A2','COL3A1', #CAF
                                       'THY1','VIM','DCN','HAS2','FN1', #fb
                                       'PDGFRB','RGS5','CD248','CSPG4','ABCC9','ACTN1', #pericytes
                                       'NT5E','ENG', 'CD44','ALCAM','NGFR','NANOG','POU5F1','SOX2','KIT', #Mesenchymal 
                                       'PPARG','ADIPOQ', #adipocytes
                                       'ACTA2','TAGLN','MYL9','MYH11', #myo
                                       'CNN1','MYLK',#vSM
                                       'MKI67' #cycling
),
group.by = 'integrated_snn_res.0.2', dot.scale = 5)+ scale_color_viridis() + coord_flip()+ RotatedAxis()+
...
VlnPlot(seu,paste0(names(sigs),1)[16:23],pt.size = 0)+NoLegend()

DotPlot(seu,assay = 'RNA',features = paste0(names(sigs),1),
        group.by = 'integrated_snn_res.0.2', dot.scale = 3)+ 
...
seu$cell_type_fine <- ifelse(seu$integrated_snn_res.0.2 %in% c(6), 'Cycling cells', 'NA')
seu$cell_type_fine <- ifelse(seu$integrated_snn_res.0.2 %in% c(0,2,7,8), 'CAFs', seu$cell_type_fine)
seu$cell_type_fine <- ifelse(seu$integrated_snn_res.0.2 %in% c(3), 'CAFs (ECM)', seu$cell_type_fine)
seu$cell_type_fine <- ifelse(seu$integrated_snn_res.0.2 %in% c(1), 'CAFs (cytokines)', seu$cell_type_fine)
seu$cell_type_fine <- ifelse(seu$integrated_snn_res.0.2 %in% c(5), 'CAFs (antigen-pres.)', seu$cell_type_fine)
seu$cell_type_fine <- ifelse(seu$integrated_snn_res.0.2 %in% c(4), 'Pericytes', seu$cell_type_fine)
