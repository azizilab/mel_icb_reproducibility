...
GSEA = function(gene_list, GO_file, pval, scoreType) {
  set.seed(54321)
  library(dplyr)
  library(fgsea)
  ...
  header = paste0("Top 10 (Total pathways: Up=", total_up,
                  ", Down=",total_down, ")\n",
                  strsplit(GO_file,'/')[[1]][length(strsplit(GO_file,'/')[[1]])])
  
  colos = setNames(c("firebrick2", "dodgerblue2"),
                   c("Up-regulated", "Down-regulated"))
  
  filtRes$pathway<- tolower(filtRes$pathway)
  filtRes$pathway<- sub("(?<=^.{60}).*","", filtRes$pathway, perl = T)
...
plot_geneset_clusters = function( gs_results, GO_file, min.sz = 4, main="GSEA clusters"){
  ...
  g1 = ggplot(gs_results, aes(x = Cluster, y = NES, label = Label )) +
    ...
    geom_text_repel( force = 2, max.overlaps = Inf, aes(angle=90)) +
    ggtitle(main) +
    theme_bw()
...
GO_files<- c("genesets/h.all.v2023.1.Hs.symbols.gmt",
             "genesets/c2.cp.kegg.v2023.1.Hs.symbols.gmt",
             "genesets/c2.cp.reactome.v2023.1.Hs.symbols.gmt",
             "genesets/c2.cp.wikipathways.v2023.1.Hs.symbols.gmt",
             "genesets/c5.go.bp.v2023.1.Hs.symbols.gmt",
             "genesets/c5.go.mf.v2023.1.Hs.symbols.gmt",
             "genesets/c8.all.v2023.1.Hs.symbols.gmt",
             "genesets/wouters.txt"
             # "genesets/wouters_mel_sigs.csv"
             )
